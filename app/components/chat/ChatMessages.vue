<template>
  <div v-if="contact" class="relative w-full h-full overflow-hidden">
    <div
      ...
      :style="{ opacity: hrequestseaderOpacity }"
    >
      <div
        v-if="floatingHeader"
                ...
                <MedicSelector mode="medic">
                  <template #trigger>
                    ...
                    >
                      <BIcon
                        icon="PhUserPlus"
                        class="w-5 h-5 fill-on-surface/50"
                      />
                      <div class="text-body-sm select-none text-on-surface/70">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      unsubPersonalInfo = chatActionStore.personalInfoBus.on(
        (conversationId) => {
    ...
    const confirmPersonalInfoRequest = () => {
      if (pendingRequestConversationId.value) {
        chatActionStore.sendPersonalInfoRequest(
          pendingRequestConversationId.value,
        );
    ...
    const handleOption = (key: string) => {
      switch (key) {
        case "prescribe-meds":
          if (
            props.contact?.nationalCode &&
            props.contact.nationalCode.trim().length > 0
          ) {
