<template>
    <Teleport to="body">
        <div @click.self="closeImage"
            :class="[isOpen ? ' md:bg-black/10 dark:md:bg-white/10 bg-black dark:bg-white md:backdrop-blur-lg pointer-events-auto visible opacity-100' : 'md:bg-black/0 dark:md:bg-white/0 bg-black/0 dark:bg-white/0 backdrop-blur-none pointer-events-none invisible opacity-0']"
            class=" transition-all flex flex-col duration-200 ease-in-out fixed top-0 left-0 z-100 w-dvw h-dvh">

            <div :style="isMobile ? { transform: `translateY(${translateY}px)` } : {}"
                :class="[isDragging ? 'transition-none' : 'transition-transform duration-300 ease-in-out']"
                class="w-full h-full flex flex-col">

                <div class=" w-full p-3 flex justify-between items-center pointer-events-auto">
                ...
                <div @click.self="closeImage" @touchstart.self="startDrag" @mousedown.self="startDrag"
                    ...
                    <div @click.self="closeImage" @touchstart.self="startDrag" @mousedown.self="startDrag"
                        ...
                        <div @click.self="closeImage" @touchstart.self="startDrag" @mousedown.self="startDrag"
                            ...
                            <BCarousel v-if="isMobile && images.length > 0 && selectedImage !== -1" :items="images"
                                v-model="selectedImage" class="w-full h-full pointer-events-auto" @touchstart.stop
                                @mousedown.stop>
                                <template #slide="{ item }">
                    ...
                    <div v-if="images.length > 1"
                        ...
                        :class="[isOpen ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0']" @touchstart.stop
                        @mousedown.stop>
                        ...
                        <div @click="selectImage(index)"
                            ...
                            <BImage fit="cover" :src="image" class="w-full h-full pointer-events-none" />
                        </div>
...
export default defineComponent({
    ...
    setup(props, { expose }) {
        ...
        // --- Core Drag Logic (Mapped exactly from your ThePopup) ---
